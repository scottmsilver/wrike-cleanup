...
from preview.wrike import WrikeApi
...
def upload_file_to_google_drive(drive_service, filename, folder_id):
    ...
    uploaded_file = (
        drive_service.files().create(body=file_metadata, media_body=MediaFileUpload(filename), fields="id").execute()
    )
...
def process_attachment(drive_service, wrike_api, attachment, folder_id):
    ...
    try:
        ...
        response = wrike_api.add_comment(
            attachment, f"Moved attachment {attachment['name']} to Google Drive: {shareable_link}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--wrike_config_json", type=str, default="config.json", help="The default path to the config json for Wrike."
    )
